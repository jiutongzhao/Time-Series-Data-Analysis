...
The **trace spectrum** — the sum of the PSDs of the three field components $B_x,B_y,B_z$ — shows four regimes separated by two breaks:

- an **energy-injection ("$1/f$") range** at the lowest frequencies (below $\sim 10^{-3}\,\mathrm{Hz}$ at 1 AU), with slope near $f^{-1}$, where large-scale energy is fed into the system;
...
- an **ion-kinetic (dissipation) range** beyond the **ion-scale spectral break** (near $\sim 0.1$–$0.4\,\mathrm{Hz}$ at 1 AU, set by the proton gyroradius / inertial length), where the spectrum steepens to the near-universal slope $f^{-2.8}$ (Alexandrova et al. 2009);
- a steep **electron-scale range** beyond a second break near a few $\times 10\,\mathrm{Hz}$, where the cascade reaches electron scales and the spectrum drops off steeply (here $\sim f^{-4}$).

We can reproduce this whole structure with exactly the Fourier-shaping trick used above for colored noise: draw a complex Gaussian spectrum, multiply it by the square root of the target piecewise power law, and inverse-transform. Synthesising three independent components and estimating the trace PSD with **Welch's method** recovers the imposed slopes — a clean numerical model of the measured solar-wind spectrum.
...
:::
<p align = 'center'>
<i>A synthetic model of the solar-wind magnetic turbulence spectrum (after Verscharen, Klein &amp; Maruca 2019): a $1/f$ injection range, a Kolmogorov $f^{-5/3}$ inertial range, a steeper $f^{-2.8}$ ion-kinetic range beyond the ion break, and a steep electron range beyond the electron break.</i>
</p>

A least-squares fit on the log–log spectrum returns $-1.68$ in the inertial range, $-2.80$ in the ion-kinetic range, and $-4.0$ beyond the electron break, matching the imposed exponents. On real data the same Welch-plus-slope-fit pipeline is the standard tool for measuring the **spectral index** and locating the spectral **breaks** — the only change is that `bx, by, bz` are read from a magnetometer file instead of being synthesised.
