...
def kalman(use_gyro=True, qa=3.0, sig=8.0, outage=None):
    ...
    for k, tk in enumerate(T):
        ...
        Fm = np.eye(4); Fm[0, 2] = Fm[1, 3] = dt; Fm[2:, 2:] = R
...
![From raw pseudoranges to a clean track. (a) The raw single-point-positioning fixes (red dots) scatter around the road; the Kalman-fused track (blue) is smooth, laid over the real street map. Boxes *b* and *c* mark the two zooms. (b) Street-level zoom: the raw SPP fixes bounce ±10–20 m off the road while the fused track stays on it. (c) A simulated 10 s GPS outage through an 83° bend: with the gyroscope the filter dead-reckons around the corner and stays on the road (blue), while a plain constant-velocity coast (grey) drives straight off the hillside.](../Figure/figure_kalman_gps_imu.png)
</div>

Now that the input is genuinely raw, the filter has real work to do. With GNSS present (a, b) the fused track sits a median **5 m** off the raw fixes — it is actively pulling the scatter back onto the road and interpolating the sparse one-per-second fixes into a continuous 4 Hz line. The dramatic gain is the **10 s outage** (c): withholding every fix across an 83° bend, the gyro-steered filter rounds the corner and finishes **≈ 19 m** from truth, while the identical filter *without* the gyro coasts straight ahead and ends **≈ 161 m** off — out on the hillside. That is the tunnel/underpass problem, and it is why phones fuse GNSS with their IMU.

A few honest caveats. This is a single-frequency solution with no ionosphere or troposphere correction, so the raw scatter is larger than a survey receiver's — which only makes the filtering more visible. The gyro was calibrated once against the SPP course, and only heading *changes* are integrated (a scale-and-bias fit, not a magnetometer heading). The outage is *simulated* by hiding good fixes, which lets us score the dead-reckoning against known truth. And dead reckoning is a loan, not a gift: the error in (c) keeps growing, so the trick buys tens of seconds of graceful coasting — enough to cross a bridge — not minutes.
