...
**What the gyroscope does and doesn't do.** A gyro measures only turn *rate* — it carries no absolute position, so it cannot beat GNSS accuracy while satellites are in view; panels (a, b) are as accurate as the pseudoranges allow, no better. Its value is twofold. During the outage (c) it is decisive — without heading, the dead-reckoned track cannot turn. And even with GNSS present it earns its keep indirectly: because the *prediction* now curves with the road, the filter can be trusted enough to smooth hard through bends without cutting the corner. That trust is a genuine tradeoff — dialling the smoothing up (lower process noise) tightens the line in (b) but makes the velocity, and hence the dead reckoning in (c), a little laggier; the settings here (`qa=1`) sit deliberately in the middle.
