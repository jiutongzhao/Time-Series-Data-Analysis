...
**Companion:** [Electromagnetic Waves — Solved with the FFT](spectral_em.qmd)
...
That is the appeal of spectral methods: a hard, spatially-coupled PDE becomes pointwise multiplication in $k$-space. The same trick — $\nabla \to i\mathbf k$ — solves Poisson's equation (divide by $-k^2$), advection, the wave equation, and, with the nonlinear term handled pseudo-spectrally, even Navier–Stokes. The heat equation is the *easiest* case because it has a real, decaying exponential per mode; the [electromagnetic-wave companion](spectral_em.qmd) shows what happens when that exponential becomes **oscillatory** — Maxwell's curl equations solved the same spectral way.
