...
### Application: Hearing Direction with Two Channels (ITD & ILD)

Your two ears are a two-channel array, and your brain runs cross-spectral analysis on them constantly. A source at azimuth $\theta$ reaches the nearer ear first — the ***interaural time difference*** $\tau(\theta) \approx 0.66\,\mathrm{ms}\cdot\sin\theta$ — and the head's acoustic shadow makes it louder on that side (the ***interaural level difference***). In cross-spectral language, a pure delay is a **phase of the cross-spectrum growing linearly with frequency**,

$$
\arg\left(S_L S_R^*\right) = 2\pi f\,\tau(\theta),
$$

so the direction can be read straight off the phase-difference spectrum: its slope is the delay, its sign is the side. Rayleigh's *duplex theory* splits the work — below ~1.5 kHz the phase is unambiguous and **ITD dominates**; above, the phase wraps (spatial aliasing) but the head shadow is strong, and **ILD takes over**.

Both cues share a blind spot: $\sin\theta = \sin(180°-\theta)$, so a source in front and its mirror behind produce **identical ITD and ILD** — the *cone of confusion*. No interaural comparison can break the tie. What does is a **monaural** cue: the outer ear (the ***pinna***) filters sound differently by direction — a source behind you loses treble as it diffracts around the folded cartilage. Direction is not only *where the two channels differ*, but also *how a single channel is coloured*.

The demo below imprints a *known* trajectory — the speaker **circles the listener once every 25 s** — onto a mono voice recording: time-varying fractional delay (ITD) + level difference (ILD) + head-shadow low-pass on the far ear + a simplified pinna cue (a rear high-shelf, −11 dB above 4 kHz straight behind). **Wear headphones.** The two *difference* spectra oscillate once per revolution and repeat exactly on the front and back halves — the cone of confusion, visualised — while the pinna cue appears only in the single-channel spectrogram, as a high-frequency dip at every rear pass. For one full revolution (between the dashed lines) the pinna cue is switched **off**, with the switches placed at front passes where the filter is the identity, so only the rear half changes: listen for the voice's orbit collapsing into an ambiguous left–right sweep:
...
rear = (1 - np.cos(phi))/2                           # 0 in front .. 1 behind
low4 = sig.filtfilt(*sig.butter(4, 4000, fs=fs), mono)
src  = low4 + (1 - 0.72*rear)*(mono - low4)          # pinna cue: rear high-shelf

tau = 0.66e-3*np.sin(phi)                            # ITD  (same for front & back!)
delay = lambda x, d: np.interp(t - d, t, x)          # time-varying fractional delay
L = 10**(-4*np.sin(phi)/20) * delay(src, +tau/2)     # ILD gain + delayed copies
R = 10**(+4*np.sin(phi)/20) * delay(src, -tau/2)

_, _, SL = sig.stft(L, fs, nperseg=4096, noverlap=4096-2205)
...
Real pinnae are richer than a shelf — they imprint individual, elevation-dependent notches that also encode *up vs. down* — and real listeners additionally break the cone of confusion by moving their heads. See the [gallery example](gallery/sound_localization.qmd) for the full construction and the self-check that recovers the orbit, the ITD, and the pinna cue back out of the rendered audio.
...
flowchart TD
  ...
  A --> B["\⟨B\⟩<br/>Avg Background Field"]

  %%=== Time–Frequency Transform ===%%
  A -->|STFT or CWT| C["B̂_i(f,n)"]
  
  %%=== Cross–Spectral Estimation ===%%
  subgraph Matrix["Construct Spectral Matrix"]
  	direction LR
  	C -->|Outer Product| D["B̂_i B̂_j^*"]
  	D -->|Smoothing time freq| E["S_ij(f)"]
  end
  %%=== Spectral Decomposition ===%%
  E -->|Form 6×3| F["S_6 times3"]
  F -->|Real Part| G["ReS_3 times3"]
  F -->|Imag Part| H["ImS_3 times3"]

  %%=== Analysis Methods ===%%
  F -->|SVD<br/>Santoňlík et al. 2003| I["k, F, WNA"]
  G -->|Eigenanalysis<br/> McPherron 1972| J["k,  WNA"]
  H -->|Eigenanalysis<br/> Means 1972| K["k,  WNA"]
  E -->|FAC| L["B_\∥, B_LH, B_RH,  epsilon_B,  delta B_\∥/ delta B"]
  B -->|Combine background| L

  %%=== Wave Frame Analysis ===%%
  E -->|Combine matrix| M["S_wf(3 times3)"]
  I -->|Combine k| M
  M --> N["ReS_wf"]
  M --> O["Coherency"]
  M --> P["DOP"]
