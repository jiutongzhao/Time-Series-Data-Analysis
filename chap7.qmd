...
- One panel per detector — **LIGO Hanford** (top) and **LIGO Livingston** (bottom). Each shows the **Morlet-wavelet (CWT) scalogram** of that detector's whitened strain, with the band-passed strain itself drawn as the wiggling line near the bottom.
...
- Because the wavelet's window *narrows in time as the frequency rises* (constant-$Q$), it tracks the rapidly accelerating chirp far more cleanly than a fixed-window STFT could — a preview of the continuous wavelet transform introduced next.
:::
...
import scipy.signal as sig, scipy.interpolate, ssqueezepy
...
# one panel per detector: CWT scalogram + the whitened strain overlaid at the bottom
fig, (axH, axL) = plt.subplots(2, 1, figsize=(9, 6), sharex=True)
fig.subplots_adjust(hspace=0.06)
for ax, raw, cmap, wcol, label in [(axH, H1, cmap_H, '#ff9a2e', 'LIGO Hanford'),
                                   (axL, L1, cmap_L, '#46a6ff', 'LIGO Livingston')]:
    Xw = whiten(raw)
    mw = (t > tm - 0.60) & (t < tm + 0.12)
    xw, tw = Xw[mw], t[mw] - tm
    Wx, scales = ssqueezepy.cwt(xw, ('morlet', {'mu': 6}), fs=fs, l1_norm=False)
    freqs = scale_to_freq(scales, ('morlet', {'mu': 6}), len(xw), fs=fs)
    sel = (freqs >= 18) & (freqs <= 512); A = np.abs(Wx[sel])
    ax.pcolormesh(tw, freqs[sel], A, shading='gouraud', cmap=cmap,
                  vmax=np.percentile(A[:, np.abs(tw) < 0.18], 99.6))
    w = sig.filtfilt(bb, ab, Xw)[mw]; w = np.clip(w / (4 * w.std()), -3.2, 3.2)
    ax.plot(tw, 22 * 10**(0.115 * w), color=wcol, lw=0.9)     # strain overlaid near 22 Hz
    ax.set_yscale('log'); ax.set_ylim(18, 512); ax.set_xlim(-0.42, 0.05)
    ax.set_yticks([32, 64, 128, 256, 512]); ax.set_yticklabels([32, 64, 128, 256, 512])
    ax.text(0.015, 0.12, label, transform=ax.transAxes, color=wcol, fontsize=13,
            fontweight='bold', va='center')
    ax.set_facecolor('black')
axL.set_xlabel('Time from merger [s]'); fig.supylabel('Frequency [Hz]')
