...
A spectrogram plots only $|S(f,t)|$ — the STFT **magnitude**. The phase is discarded, and with it, apparently, the waveform. But an STFT with overlapping windows is **redundant**: at 75 % overlap every sample is described by four different frames, so the magnitude alone holds roughly *four numbers per signal sample*. Neighbouring frames must agree about the samples they share, and essentially only one phase assignment lets them — the redundancy carries the phase in disguise.
...
<!-- fig-tabset:figure_griffin_lim.png -->
::: {.panel-tabset}

### Figure

<img src="Figure/figure_griffin_lim.png" width="100%"/>
...
:::
<p align = 'center'>
    Speech recovered from a magnitude-only spectrogram. Overlap is what makes it work: without redundancy (right curve), any phase is self-consistent and the iteration never improves.
</p>
...
<tr>
  <td style="border:none;"><b>original</b></td>
  <td style="border:none;"><b>iteration 1</b><br><small>random phase</small></td>
  <td style="border:none;"><b>iteration 5</b></td>
...
The same "phase problem" rules X-ray crystallography, where detectors record only $|\mathcal{F}\{\rho\}|^2$; and every neural vocoder in modern text-to-speech is trained to solve exactly this task — spectrogram in, phase out. See the [gallery example](gallery/spectrogram_inversion.qmd) for the full derivation, the overlap comparison, and the honest limits.
...
plt.savefig(save_path + 'figure_ligo_gw150914.png', bbox_inches='tight', dpi=200, facecolor='white')
```
