...
- **Panel (b)** is the STFT spectrogram of H1: the signal frequency climbs from ~35 Hz past ~150 Hz in the final ~0.2 s before merger — the inspiral chirp, made visible only because the time–frequency plane localises it in both axes at once.
:::
...
# (b) STFT spectrogram — the upward chirp
f, tt, Sxx = sig.spectrogram(whiten(H1), fs=fs, nperseg=512, noverlap=505,
                             window=('tukey', 0.25))
S_db = 10 * np.log10(Sxx + 1e-12)
reg = S_db[(f > 20) & (f < 400)][:, np.abs(tt - tm) < 0.25]
ax2.pcolormesh(tt - tm, f, S_db, shading='gouraud', cmap='viridis',
               vmin=np.percentile(reg, 40), vmax=np.percentile(reg, 99.7))
ax2.set_ylim(20, 400); ax2.set_xlim(-0.16, 0.06)
