...
t0 = tm - 0.45                                              # window start -> x runs 0..0.5 s
...
yc, kk, Sbar = 30.0, 0.042, 5.0                             # trace baseline (Hz), log-gain, scale bar (sigma)
...
for ax, raw, cmap, wcol, label in [(axH, H1, cmap_H, '#ff9a2e', 'LIGO Hanford'),
    ...
    ax.pcolormesh(xw, freqs[sel], A, shading='gouraud', cmap=cmap,
                  vmax=np.percentile(A[:, xw > 0.30], 99.6))
    # whitened strain in units of sigma, shown only over the time of interest
    ws = sig.filtfilt(bb, ab, Xw)[mw]; ws = ws / ws[xw < 0.25].std()
    keep = xw > 0.33
    ax.plot(xw[keep], yc * 10**(kk * np.clip(ws[keep], -6, 6)), color=wcol, lw=1.9)
    # vertical I-beam scale bar: absolute amplitude with its unit (sigma)
    xb = 0.055
    ax.errorbar([xb], [yc], yerr=[[yc - yc*10**(-kk*Sbar)], [yc*10**(kk*Sbar) - yc]],
                fmt='none', ecolor=wcol, elinewidth=2.3, capsize=7, capthick=2.3)
    ax.text(xb + 0.018, yc, f'{Sbar:.0f}$\\sigma$', color=wcol, fontsize=12,
            va='center', ha='left', fontweight='bold')
    ax.set_yscale('log'); ax.set_ylim(18, 512); ax.set_xlim(0, 0.5)
    ax.set_yticks([32, 64, 128, 256, 512]); ax.set_yticklabels([32, 64, 128, 256, 512])
    ax.tick_params(labelsize=12)
    ax.text(0.02, 0.90, label, transform=ax.transAxes, color=wcol, fontsize=13,
