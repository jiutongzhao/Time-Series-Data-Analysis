...
### Application: Beats in Musical Triads

Superpose two sinusoids of *nearly* equal frequency and the sum is not a new pure tone — it is a tone at the mean frequency whose **amplitude slowly waxes and wanes**:

$$
\sin(2\pi f_1 t) + \sin(2\pi f_2 t) = 2\cos\!\big(\pi (f_1 - f_2)\, t\big)\,\sin\!\big(\pi(f_1+f_2)\,t\big),
$$

an envelope pulsing at the difference frequency $|f_1 - f_2|$ — the ***beat***. Musicians live inside this formula. Real instrument tones carry harmonics at $2f_0, 3f_0, \dots$, and when two notes sound together, it is their *harmonics* that land close to each other and beat. How close decides everything:

- $\Delta f \lesssim$ a few Hz — a slow, gentle shimmer (what a piano tuner listens to);
- $\Delta f \sim$ 20–60 Hz — too fast to follow, fusing into the sensation of **roughness**: this is Helmholtz's classic account of *dissonance*.

The four **triads** built on the same root C make a controlled experiment, because equal temperament detunes every interval by a different amount. In the C-major and C-minor chords the fifth C–G is nearly pure: the harmonics $3f_C = 784.9$ Hz and $2f_G = 784.0$ Hz collide just $0.9$ Hz apart — a slow beat. Replace the fifth by the diminished chord's G♭ or the augmented chord's G♯ and the nearest collisions ($3f_C \leftrightarrow 2f_{G♭}$, $3f_C \leftrightarrow 2f_{G♯}$) sit **45 Hz** apart — squarely in the roughness band:

<!-- fig-tabset:figure_triad_beats.png -->
::: {.panel-tabset}

### Figure

<img src="Figure/figure_triad_beats.png" width="100%"/>
...
f_C = 261.626                                   # C4, equal temperament
semi = lambda k: f_C * 2**(k/12)                # k semitones above the root

def tone(f0, NH=6):                             # a note with 6 harmonics
    return sum(np.sin(2*np.pi*n*f0*t + 0.7*n) / n**1.2 for n in range(1, NH+1))

chords = {'major':      [0, 4, 7],              # C  E  G
          'minor':      [0, 3, 7],              # C  Eb G
          'diminished': [0, 3, 6],              # C  Eb Gb
          'augmented':  [0, 4, 8]}              # C  E  G#
signals = {name: sum(tone(semi(k)) for k in steps)
           for name, steps in chords.items()}

# where the harmonics collide:
#   3*f_C = 784.9 Hz   vs  2*f_G  = 784.0 Hz   ->  0.9 Hz   slow beat  (maj, min)
#   3*f_C = 784.9 Hz   vs  2*f_Gb = 740.0 Hz   -> 44.9 Hz   roughness  (dim)
#   3*f_C = 784.9 Hz   vs  2*f_G# = 830.6 Hz   -> 45.7 Hz   roughness  (aug)

# isolate the colliding pair to SEE its beat: band-pass around the collision
import scipy.signal
bp = {name: scipy.signal.filtfilt(*scipy.signal.butter(4, [700, 865],
            btype='band', fs=fs), y) for name, y in signals.items()}
```

:::
<p align = 'center'>
    The same 700–865 Hz band of each chord, seen in both domains. Top: the band-passed waveform — in the major and minor triads the colliding pair beats once per second, a single wave packet longer than the panel; in the diminished and augmented triads the packets shrink to ~22 ms, repeating 45 times a second. Bottom: the band's spectrum — two lines almost merged (slow beat) versus two lines ~45 Hz apart (roughness). The packet length in time is exactly the inverse of the line spacing in frequency.
</p>

Listen to the four — same root, same synthesis, only the intervals differ. The beating you hear *is* the spectral spacing in the bottom row:

<table style="width:100%; border:none; text-align:center;">
<tr>
  <td style="border:none;"><b>major</b><br><small>C–E–G</small></td>
  <td style="border:none;"><b>minor</b><br><small>C–E♭–G</small></td>
  <td style="border:none;"><b>diminished</b><br><small>C–E♭–G♭</small></td>
  <td style="border:none;"><b>augmented</b><br><small>C–E–G♯</small></td>
</tr>
<tr>
  <td style="border:none;"><audio controls preload="none" style="width:100%; max-width:200px;"><source src="Data/triad_major.wav" type="audio/wav" /></audio></td>
  <td style="border:none;"><audio controls preload="none" style="width:100%; max-width:200px;"><source src="Data/triad_minor.wav" type="audio/wav" /></audio></td>
  <td style="border:none;"><audio controls preload="none" style="width:100%; max-width:200px;"><source src="Data/triad_diminished.wav" type="audio/wav" /></audio></td>
  <td style="border:none;"><audio controls preload="none" style="width:100%; max-width:200px;"><source src="Data/triad_augmented.wav" type="audio/wav" /></audio></td>
</tr>
</table>

Two remarks close the loop. First, the beat is the time-domain face of a frequency-domain fact — two spectral lines $\Delta f$ apart *are* an envelope at $\Delta f$; the pair of views becomes the central theme of [Chapter 2](chap2.qmd). Second, this is why a piano tuner needs no spectrum analyser: tuning a fifth means listening to $3f_1 \leftrightarrow 2f_2$ and slowing the beat down — counting beats is measuring $\Delta f$ by ear.
...
| Library  |                          Advantages                          |
| :------: | :----------------------------------------------------------: |
|  [Pandas](https://pandas.pydata.org/docs/user_guide/timeseries.html)  | Rich time series functionality, easy integration with DataFrames **[Use in Reading Table Files]** |
|  [NumPy](https://numpy.org/doc/stable/reference/arrays.datetime.html)   | Efficient for large arrays and vectorized time operations **[Use in Data Analysis]** |
| [datetime](https://docs.python.org/3/library/datetime.html) | Part of Python standard library for basic date/time operations **[Use in Timestamps Format Conversion]** |
| [Astropy](https://docs.astropy.org/en/stable/time/)  | High precision handling of special time formats and leap seconds **[Use in Time System Conversion]** |
...
[NASA CDF (Common Data Format)](https://cdf.gsfc.nasa.gov/) is the standard container for space-physics and heliospheric data (e.g. MMS, Wind, ACE, Parker Solar Probe). It defines three dedicated epoch data types:

| Type | Storage | Resolution | Reference epoch | Leap seconds |
|------|---------|------------|-----------------|--------------|
| `CDF_EPOCH` | `float64` (8 B) | 1 ms | 0000-01-01 00:00:00 UTC | ✗ |
| `CDF_EPOCH16` | 2 × `float64` (16 B) | 1 ps | 0000-01-01 00:00:00 UTC | ✗ |
| `CDF_TIME_TT2000` | `int64` (8 B) | 1 ns | 2000-01-01 12:00:00 TT (J2000) | ✓ |

- **`CDF_EPOCH`** — legacy format; stores milliseconds since year 0. Good enough for most pre-2000 datasets and still the most widely encountered type.

- **`CDF_EPOCH16`** — highest resolution (picoseconds), but costs 16 bytes per sample and stores the value as a complex number (`np.complex128`) in Python. Rarely needed in practice.

- **`CDF_TIME_TT2000`** — recommended for all new datasets. Uses **Terrestrial Time (TT)** rather than UTC, so the integer counter never has an ambiguous leap-second repeat. The offset between TT and UTC is $\Delta AT + 32.184\ \text{s}$ (where $\Delta AT$ is the accumulated leap-second count). Valid range: 1707 – 2292.
