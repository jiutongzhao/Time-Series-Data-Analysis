...
# editorial diverging palette: slate-blue -> warm cream -> terracotta
cmap = LinearSegmentedColormap.from_list(
    'editorial', ['#2c5d86', '#7da7c4', '#f3efe7', '#dca27f', '#b34b34'])
INK, FRAME = '#33312e', '#cdc7bb'

def caption(ax, name, kern):
    ax.text(0.5, -0.10, name, transform=ax.transAxes, ha='center', va='top',
            fontsize=12, color=INK, fontweight='medium')
    ax.text(0.5, -0.235, kern, transform=ax.transAxes, ha='center', va='top',
            fontsize=9, color='#8a8377')
...
def shaded_sphere(l, m, tilt=-58, azim=18, npix=620, light=(-0.45, 0.6, 0.78)):
    ...
    rgb = rgb*(0.34 + 0.66*diff)[..., None] + (diff**22*0.22)[..., None]
    return np.dstack([np.clip(rgb, 0, 1), inside.astype(float)])

plt.close()
fig = plt.figure(figsize=(8.6, 3.6), dpi=150)
gs = fig.add_gridspec(1, 3, wspace=0.14, left=0.015, right=0.985, top=0.78, bottom=0.28)
...
sq = lambda a: a.add_patch(plt.Rectangle((-1, -1), 2, 2, fill=False, ec=FRAME, lw=1.3))
ci = lambda a, lw=1.3: a.add_patch(plt.Circle((0, 0), 1, fill=False, ec=FRAME, lw=lw))

# (1) Fourier — plane wave on a square (the plane)
a = np.deg2rad(30)
ax[0].imshow(np.cos(7.5*(X*np.cos(a) + Y*np.sin(a))), **imkw)
sq(ax[0]); caption(ax[0], 'Fourier', r'$e^{\,i\mathbf{k}\cdot\mathbf{x}}$   ·   plane')

# (2) Hankel — concentric Bessel rings on the disk
Fh = jn(0, jn_zeros(0, 3)[-1]*R); Fh[R > 1] = np.nan
ax[1].imshow(Fh, **imkw); ci(ax[1]); caption(ax[1], 'Hankel', r'$J_m(kr)$   ·   disk')

# (3) Legendre / spherical harmonics — shaded sphere
ax[2].imshow(shaded_sphere(5, 3), extent=[-1, 1, -1, 1], origin='lower')
ci(ax[2], 1.0); caption(ax[2], 'Legendre', r'$P_\ell(\cos\theta)$   ·   sphere')

for a_ in ax:
    a_.set_xlim(-1.04, 1.04); a_.set_ylim(-1.04, 1.04); a_.set_aspect('equal'); a_.axis('off')

fig.text(0.5, 0.95, 'One idea, three geometries — each transform is a basis of orthogonal sign patterns',
         ha='center', va='top', fontsize=12, color=INK)
fig.text(0.5, 0.865, 'blue $-$   cream $0$   red $+$    ·    matching modes reinforce, mismatched ones cancel',
         ha='center', va='top', fontsize=8.8, color='#8a8377')
fig.savefig(save_path + 'figure_spectral_transforms_overview.png',
...
The curve above is a *prediction*. Does a physical aperture obey it? The image below is a public-domain photograph ([CC0](https://commons.wikimedia.org/wiki/File:Beugungsscheibchen.k.720.jpg), B. Bautsch) of monochromatic **red laser light** diffracted by a **0.09 mm circular pinhole**, recorded 65 mm downstream — a textbook Fraunhofer setup that resolves diffraction orders out to the 27th ring. We linearise the photograph (undo the sRGB gamma), recover the pattern centre by **minimising the azimuthal variance** (a correct centre makes the rings concentric), **azimuthally average** it into a measured radial profile, fit the *single* scale $r_\mathrm{Airy}$ to the first three dark rings, and overlay the parameter-free Airy prediction.
...
# --- centre: a centroid is biased by the saturated core, so refine it by
#     minimising the azimuthal variance (rings are concentric only at the
#     true centre).  The search runs on a 4x-downsampled copy for speed. ---
f  = 4
Id = I[::f, ::f]
ys, xs = np.indices(Id.shape)
iy, ix = np.unravel_index(np.argmax(Id), Id.shape)
cx, cy = float(ix), float(iy)
for _ in range(6):                               # intensity-centroid initial guess
    rr = np.sqrt((xs - cx)**2 + (ys - cy)**2)
    w  = np.where(rr < 250 / f, Id, 0.0)
    cx, cy = (xs * w).sum() / w.sum(), (ys * w).sum() / w.sum()

rmax_o, nb_o = 1400 / f, 180
def azim_var(p):                                 # total within-ring variance
    r = np.sqrt((xs - p[0])**2 + (ys - p[1])**2).ravel()
    m = r < rmax_o
    b = (r[m] / rmax_o * nb_o).astype(int)
    v = Id.ravel()[m]
...
cx, cy = opt.x[0] * f + (f - 1) / 2, opt.x[1] * f + (f - 1) / 2   # back to full res
...
# --- fit the single scale r_Airy to the first three dark rings ---
# (the first dark ring IS r_Airy by definition; three rings make the fit robust)
sm   = np.convolve(np.log(pn + 1e-5), np.ones(7) / 7, mode='same')
mins = argrelmin(sm, order=12)[0]
rmin = rc[mins[(rc[mins] > 80) & (rc[mins] < 480)]]      # first three clean minima
...
r_core  = rmin[0]                                        # saturated core = first dark ring
...
ax[0].plot(0, 0, '+', color='#FF3B3B', ms=7, mew=1.2)    # fitted pattern centre
...
ax[1].axvspan(0, r_core / rA, color='0.85', zorder=0)
ax[1].text(r_core / rA / 2, 3e-3, 'saturated\ncore', ha='center', va='center',
...
# a test signal along the polar axis, x = cos(theta)
f_vals = np.exp(-5 * x**2) * np.cos(3 * np.pi * x)

# forward Legendre transform -> coefficients a_l
a = (2 * ell + 1) / 2 * (P_mat @ (w * f_vals))

# inverse synthesis: full, and truncated to L modes
f_full = a @ P_mat
recon  = lambda L: a[:L] @ P_mat[:L]

BLUE, CORAL, INK, GREY = '#2c5d86', '#b34b34', '#33312e', '#8a8377'
plt.close()
fig, (axf, axs) = plt.subplots(1, 2, figsize=(8.4, 3.4))

axf.plot(x, f_vals, color=INK, lw=2.2, label=r'signal $f(\cos\theta)$')
axf.plot(x, recon(6),  color=BLUE,  lw=1.5, ls='--', label=r'$\ell\leq 5$')
axf.plot(x, recon(14), color=CORAL, lw=1.5, ls='-.', label=r'$\ell\leq 13$')
axf.set_xlim(-1, 1); axf.set_xlabel(r'$\cos\theta$'); axf.set_ylabel(r'$f$')
axf.set_title('Legendre synthesis converges', fontsize=10.5, color=INK)
axf.legend(fontsize=8, frameon=False, loc='lower center', ncol=3,
           columnspacing=1.0, handlelength=1.6)
for s in ('top', 'right'): axf.spines[s].set_visible(False)

L = 30
axs.axhline(0, color=GREY, lw=0.6)
axs.vlines(ell[:L], 0, a[:L], color=BLUE, lw=1.5)
axs.plot(ell[:L], a[:L], 'o', color=BLUE, ms=3.5)
axs.set_xlim(-0.6, L - 0.5); axs.set_xlabel(r'degree  $\ell$')
axs.set_ylabel(r'coefficient  $a_\ell$')
axs.set_title(r'Legendre spectrum  $a_\ell$', fontsize=10.5, color=INK)
axs.text(0.97, 0.93, rf'$\max|f-f_{{\rm rec}}|\sim{np.max(np.abs(f_vals-f_full)):.0e}$',
         transform=axs.transAxes, ha='right', va='top', fontsize=8.2, color=GREY)
for s in ('top', 'right'): axs.spines[s].set_visible(False)

fig.suptitle(r'Legendre transform: a band-limited spectral decomposition on $[-1,1]$',
             fontsize=11, color=INK, y=1.0)
plt.tight_layout(rect=[0, 0, 1, 0.96])
plt.savefig(save_path + 'figure_legendre.png', bbox_inches='tight', dpi=150)
```
...
INK, BLUE, CORAL, GREEN, GREY = '#33312e', '#2c5d86', '#b34b34', '#2a8f6b', '#8a8377'
plt.close()
fig, ax = plt.subplots(1, 2, figsize=(8.8, 3.8))
o = np.argsort(theta); thd = np.rad2deg(theta[o])
ax[0].plot(thd, C_theta_exact[o], color=INK, lw=1.8, label=r'exact  $C(\theta)$  ($\Lambda$CDM)')
ax[0].plot(thd, C_theta[o], '.', color=CORAL, ms=2.4, alpha=0.55, label='Monte-Carlo pairs')
...
ax[0].set_title('(a) two-point correlation', fontsize=10); ax[0].legend(fontsize=7.5, frameon=False)
...
ax[1].plot(ell[2:], Dl(Cl_C)[2:], color=GREEN, lw=1.5, ls='--', label='fast Legendre transform')
ax[1].plot(cA, DA, 'o', color=BLUE, ms=3.2, label='anafast (Method A)')
ax[1].plot(cB, DB, 's', color=CORAL, ms=3.8, label='Wiener–Khinchin (Method B)')
ax[1].axvspan(280, lmax, color='0.93', zorder=0)
ax[1].set_xlim(0, lmax); ax[1].set_ylim(0, 6500)
ax[1].set_xlabel(r'multipole  $\ell$'); ax[1].set_ylabel(r'$D_\ell$  [$\mu$K$^2$]')
ax[1].set_title('(b) angular power spectrum', fontsize=10); ax[1].legend(fontsize=7.2, frameon=False)
for s in ('top', 'right'): ax[1].spines[s].set_visible(False)
plt.tight_layout()
plt.savefig(save_path + 'figure_wk_cmb_sphere.png', bbox_inches='tight', dpi=150)
```
