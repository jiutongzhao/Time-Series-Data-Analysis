...
# trustworthy at low l. Run it on a SMALL Gauss-Legendre grid reaching just past the
# first acoustic peak -- this also keeps the ring-sampling loop cheap.
lmax_B  = 300
ell_B   = np.arange(lmax_B + 1)
mu, wmu = np.polynomial.legendre.leggauss(lmax_B + 1)   # low-l GL nodes/weights
theta   = np.arccos(mu)
n_pairs = 200_000                                       # pixel pairs per node (SNR knob)
...
# (B2) exact Legendre transform on the low-l GL grid: C_l = 2*pi * sum_i w_i C(theta_i) P_l(mu_i)
P_B  = eval_legendre(ell_B[:, None], mu[None, :])            # (lmax_B+1, lmax_B+1)
...
plt.close()
fig, ax = plt.subplots(1, 2, figsize=(8.8, 3.9), layout='constrained')
...
ob = np.argsort(theta)
ax[0].plot(np.rad2deg(theta[ob]), C_theta[ob], '.', color=RED, ms=3.5, alpha=0.6, label='Monte-Carlo pairs')
...
ax[1].axvspan(lmax_B, lmax, color='0.93', zorder=0)
...
fig.suptitle('CMB Angular Power Spectrum', fontsize=14, fontweight='bold', fontstyle='italic', color='#222222')
