...
def caption(ax, name, kern):
    ax.text(0.5, -0.10, name, transform=ax.transAxes, ha='center', va='top',
            fontsize=11.5, color=INK, fontweight='medium')
    ax.text(0.5, -0.245, kern, transform=ax.transAxes, ha='center', va='top',
            fontsize=8.8, color='#8a8377')
...
gs = fig.add_gridspec(1, 4, wspace=0.16, left=0.012, right=0.988, top=0.80, bottom=0.30)
...
# (4) Hermite — Gaussian-localised oscillator blobs on a square
def psi(n, x):
    return (2.0**n*factorial(n)*np.sqrt(np.pi))**-0.5*eval_hermite(n, x)*np.exp(-x**2/2)
xg = np.linspace(-2.9, 2.9, 600); Xh, Yh = np.meshgrid(xg, xg)
Fherm = psi(2, Xh)*psi(2, Yh); Fherm /= np.max(np.abs(Fherm))
ax[3].imshow(Fherm, **imkw); sq(ax[3])
caption(ax[3], 'Hermite', r'$H_n(x)\,e^{-x^2/2}$   ·   line')

for a_ in ax:
    a_.set_xlim(-1.04, 1.04); a_.set_ylim(-1.04, 1.04); a_.set_aspect('equal'); a_.axis('off')

fig.text(0.5, 0.95, 'One idea, four geometries — each transform is a basis of orthogonal sign patterns',
         ha='center', va='top', fontsize=12, color=INK)
fig.text(0.5, 0.875, 'blue $-$   cream $0$   red $+$    ·    matching modes reinforce, mismatched ones cancel',
         ha='center', va='top', fontsize=8.6, color='#8a8377')
